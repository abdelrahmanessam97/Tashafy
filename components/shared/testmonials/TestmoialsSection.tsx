...
export function TestmoialsSection({ locale, title, subtitle, durationLabel, countryLabel, testimonials }: TestmoialsSectionProps) {
  ...
  return (
    <section
      className="relative w-full overflow-hidden bg-background py-16 md:py-24 [content-visibility:auto] [contain-intrinsic-size:auto_600px]"
      dir={isRtl ? "rtl" : "ltr"}
      aria-label={title}
    >
      <SectionContainer className="relative">
        {/* Top row: title + quote | description + nav + pagination */}
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:gap-8 mb-12 md:mb-16">
          <h2 className="flex flex-1 items-center gap-2 text-2xl md:text-4xl lg:text-5xl font-bold text-(--color-text-brand)  text-start w-full md:max-w-70 ltr:max-w-[75%]">
            {title}
          ...
          <div
            className={cn(
              "flex flex-col gap-4 px-2 md:px-8 flex-1 max-w-[652px] w-full",
              isRtl ? "items-end justify-end text-right" : "items-start justify-start text-left",
            )}
          >
            <p className={cn("text-base font-medium text-(--color-text-secondary)  w-full max-w-[1000px]", isRtl ? "text-right" : "text-left")}>{subtitle}</p>
            <div className={cn("flex justify-center gap-5 w-50")}>
              <button
                type="button"
