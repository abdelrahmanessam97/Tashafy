...
export function OurPartners({ locale, title, subtitle, partners }: OurPartnersProps) {
    ...
    <section className="relative w-full overflow-hidden bg-muted/40 py-16 md:py-24" dir={isRtl ? "rtl" : "ltr"}>
      <div className="relative w-[96%] mx-auto container-padding">
        <div className={cn("mb-12 md:mb-16", isRtl ? "text-right" : "text-left")}>
          <h2 className="relative text-3xl md:text-4xl lg:text-5xl font-bold text-[#1f242e] dark:text-white mb-3 inline-flex items-center gap-2 flex-wrap">
            <Image
              src="/cursor.svg"
              alt=""
              width={28}
              height={28}
              className={cn("shrink-0 absolute top-9", !isRtl ? "-right-10 transform rotate-270" : "-left-8")}
              aria-hidden
            />
            {title}
          </h2>
