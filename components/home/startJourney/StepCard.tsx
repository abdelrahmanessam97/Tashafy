  ...
  return (
    <Card className="relative border-0 bg-white rounded-2xl shadow-[0_10px_40px_rgba(34,0,86,0.06)] px-4 py-5 md:px-6">
      <div className={cn("flex items-center gap-4")}>
        <div className={cn("shrink-0 inline-flex h-14 w-14 items-center justify-center rounded-lg bg-(--brand) text-(--text-brand) ")}>
          <Icon className="h-8 w-8" aria-hidden />
        </div>
        <div className={cn("space-y-1 min-w-0 flex-1", isRtl ? "text-right" : "text-left")}>
          <p className="text-2xl font-bold text-(--text-brand)">{step.label}</p>
